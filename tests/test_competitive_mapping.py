...
def test_cli_entry_point(samples, myco_manifest, test_outputs_dir, mocker):
    ...
    match samples["sample"]:
        case "chloro_10k":
            check_length(
                output_root + "M.tuberculosis_reads_for_assembly_1.fastq.gz", 13
            )
            check_length(
                output_root + "M.tuberculosis_reads_for_assembly_2.fastq.gz", 13
            )
            check_order(
                output_root + "M.tuberculosis_reads_for_assembly_1.fastq.gz",
                "5a36937177ce534d20988350a583c445b3dca532f8c359c285da324f95f75346",
            )
            check_order(
                output_root + "M.tuberculosis_reads_for_assembly_2.fastq.gz",
                "5a36937177ce534d20988350a583c445b3dca532f8c359c285da324f95f75346",
            )
        case "tb_10k":
            check_length(
                output_root + "M.tuberculosis_reads_for_assembly_1.fastq.gz", 5235
            )
            check_length(
                output_root + "M.tuberculosis_reads_for_assembly_2.fastq.gz", 5235
            )
        case "tb_ont":
            check_length(
                output_root + "M.tuberculosis_reads_for_assembly.fastq.gz", 438
            )
...
def test_output_fastqs_single_reference(mock_run, mock_contigs_df, temp_output_dir):
    ...
    mock_run.assert_any_call(
        f"samtools view -h {aln_bam} -u r1 | samtools sort -n -@ 1 -o {temp_output_dir}ref1_output_aln.bam",
        shell=True,
        check=True,
...
def test_output_fastqs_ont_platform(mock_run, mock_contigs_df, temp_output_dir):
    ...
    mock_run.assert_any_call(
        f"samtools fastq --excl-flags 0x100 -@ 1 -0 {temp_output_dir}ref1_reads_for_assembly.fastq.gz {temp_output_dir}ref1_output_aln.bam",
        shell=True,
        check=True,
        stdout=subprocess.PIPE,
    )
